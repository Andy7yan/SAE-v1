...
def launched_with_torchrun() -> bool:
    return int(os.environ.get("WORLD_SIZE", "1")) > 1 and "RANK" in os.environ and "LOCAL_RANK" in os.environ
...
def main() -> None:
    ...
    # Auto-use all visible GPUs on a single node when not launched by torchrun.
    port = find_free_port()
